...
class BayesianOptimization(BaseOptimization):
    def __init__(self,sorter,recording,gt_sorting, params_to_opt,space=None,run_schedule=[150,100],metric ='accuracy',recdir=None,
                 outfile='results'):
        BaseOptimization.__init__(self, sorter=sorter, recording=recording, gt_sorting=gt_sorting,
                                  params_to_opt=params_to_opt,space=space,run_schedule=run_schedule,metric =metric,recdir = None, outfile=outfile)
        self.results_obj = None

    def run(self):
        results = self.optimise(
                self.params_to_opt, self.function_wrapper, self.run_schedule)
        best_parameters = {key: results.x[i] for i, key in enumerate(self.params_to_opt)}
